...
if [ -z $NOSIGN ]; then
  echo "==> Signing..."
  pushd ./pkg/dist
  rm -f ./vault-ssh-helper_${VERSION}_SHA256SUMS*
  shasum -a256 * > ./vault-ssh-helper_${VERSION}_SHA256SUMS
  gpg --default-key 348FFC4C --detach-sig ./vault-ssh-helper_${VERSION}_SHA256SUMS
  popd
fi
...
if [ ! -z $HC_RELEASE ]; then
  ...
  curl -X PURGE https://releases.hashicorp.com/vault-ssh-helper/${VERSION}
  for FILENAME in $(find $DIR/pkg/dist -type f); do
    FILENAME=$(basename $FILENAME)
    curl -X PURGE https://releases.hashicorp.com/vault-ssh-helper/${VERSION}/${FILENAME}
  done
fi
